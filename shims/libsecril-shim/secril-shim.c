...
{
    pData = (int)data;
    RLOGD("%s: BEGIN -----------------------------------------------", __func__);
    RLOGD("%s: REQUEST %s(%d): data:%p datalen:%d RIL_Token:%d", __func__, requestToString(request), request, data, datalen, t);
    switch (request) {
        ...
        case RIL_REQUEST_RADIO_POWER:
            RLOGD("%s: REQUEST %s(%d): Passing pData:%p", __func__, requestToString(request), request, &pData);
            mRealRadioFuncs->onRequest(request, &pData, datalen, t);
            break;
        default:
            RLOGD("%s: REQUEST %s(%d): Passing data:%p", __func__, requestToString(request), request, &pData);
            mRealRadioFuncs->onRequest(request, data, datalen, t);
    }
    RLOGD("%s: END -------------------------------------------------", __func__);
}

static RIL_RadioState rilOnStateRequest() {
    RLOGD("%s: BEGIN", __func__);
    RIL_RadioState ret = mRealRadioFuncs->onStateRequest();
    RLOGD("%s: END RIL_RadioState:%d", __func__, ret);
    return ret;
}

static int rilSupports(int requestCode) {
    RLOGD("%s: BEGIN requestCode:%d ", __func__, requestCode);
    RIL_RadioState ret = mRealRadioFuncs->supports(requestCode);
    RLOGD("%s: END result:%d", __func__, ret);
    return ret;
}

static void rilCancel(RIL_Token t) {
    RLOGD("%s: BEGIN RIL_Token:%d ", __func__, t);
